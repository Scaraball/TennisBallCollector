...
# ----------------------- Callback for ROS Topics -----------------------
	...
	def clbk_balls(self, msg):  # type pose array
		for i in range(len(msg.poses)):
			...
			if self.going_to_ball:
				old_desired = np.array([[self.desired_position.x], [self.desired_position.y]])
				new_desired = np.array([[self.balls[self.ball_index][0]], [self.balls[self.ball_index][1]]])
				if np.linalg.norm(old_desired-new_desired) >= 1:	
					self.desired_position = Point(x=new_desired[0, 0], y=new_desired[1, 0])
					self.change_state(state=0)
	...
	def clbk_rob(self, msg):  # pose
		return
		print("Rob position: (", msg.position.x , ",", msg.position.y, ")")
