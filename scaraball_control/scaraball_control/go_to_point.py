...
# ----------------------- Callback for ROS Topics -----------------------
	...
	def move_to(self):
		...
		a = 0.25*np.sin(self.yaw) / (0.25*np.cos(self.yaw))
		u = a * np.ones((2, 1))
		...
		for qhat in self.obstacles: w = w + (p - qhat) / np.linalg.norm(p - qhat) ** 4
		...
		for qhat in self.net: w = w + (p - qhat) / np.linalg.norm(p - qhat) ** 4
